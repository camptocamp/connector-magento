...
class customer_credit_warning(osv.osv_memory):
    ...
    # glue between c2c_customer_credit_limit and sale_exceptions
    # as we cannot inherit, copy paste the entire osv_memory...
    ORDER_EXCEPTION = _("Cannot confirm the order, please check the exceptions.")
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        for sale in self.pool.get('sale.order').browse(cr, uid, context.get('active_ids', [])):
            if not self.pool.get('sale.order').test_exceptions(cr, uid, [sale.id]):
                return 'exceptions'
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form', context={}, toolbar=False):
        ...
        for sale in self.pool.get('sale.order').browse(cr, uid, context.get('active_ids', [])):
            ...
            elif default == 'exceptions':
                xml += '''<label string="%s"/>\n''' % (self.ORDER_EXCEPTION,)
            else:
                res_partner = partner_obj.search(cr, uid, [('id', '=', sale.partner_id.id)])
                partner = partner_obj.browse(cr, uid, res_partner)[0]
