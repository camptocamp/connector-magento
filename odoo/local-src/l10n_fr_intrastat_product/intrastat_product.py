...
class report_intrastat_product(orm.Model):
    ...
    def create_intrastat_product_lines(
            ...
            if not parent_values['is_fiscal_only']:
                ...
                if not source_uom:
                    ...
                    if source_uom.id == kg_uom_id:
                        ...
                        errors[line.id].append(
                            _("Conversion from unit of measure '%s' to "
                            "'Kg' is not implemented yet.") % (source_uom.name)
                        )
                ...
                if not product_intrastat_code:
                    errors[line.id].append(
                        _("Missing H.S. code on product '%s' "
                            "or on it's related category '%s'.") %
                        (line.product_id.name,
                    ...
                    if not product_intrastat_code.intrastat_code:
                        errors[line.id].append(
                            _("Missing intrastat code on H.S. code "
                              "'%s' (%s).") %
                ...
                if not product_intrastat_code.intrastat_uom_id:
                    ...
                    if intrastat_uom_id_to_write == source_uom_id_to_write:
                        ...
                        errors[line.id].append(
                            _("On invoice '%s', the line with product "
                              "'%s' has a unit of measure (%s) which can't "
                              "be converted to UoM of it's intrastat "
                              "code (%s).") % (invoice.number,
                                               line.product_id.name,
                ...
                # If we have the country of origin on the product -> take it
                    ...
                    if origin_partner_id:
                        ...
                        if not supplier_ids:
                            errors[line.id].append(
                                _("Missing country of origin on product "
                                  "'%s' or on it's supplier information for "
                                  "partner '%s'.") %
                                (line.product_id.name,
                                 parent_values.get('origin_partner_name', 'none'))
                            )
                        else:
                        ...
                        errors[line.id].append(
                            _("Missing country of origin on product "
                              "'%s' (it's not possible to get the country "
                              "of origin from the 'supplier information' "
                              "in this case because we don't know the "
        ...
        if lines_to_create:
            ...
            if parent_values['is_vat_required']:
                ...
                partner_country = (invoice.partner_id.country_id or
                                   invoice.partner_id.commercial_partner_id.country_id)
                if invoice.intrastat_country_id and not partner_country.intrastat:
                    if not invoice.partner_id.intrastat_fiscal_representative:
                        invoice_errors.append(
                            _("Missing fiscal representative for partner "
                              ...
                              "the goods were delivered to or received from "
                              "inside the EU.") %
        ...
        for line_to_create in lines_to_create:
            ...
            if not errors and not invoice_errors:
                line_obj.create(cr, uid, line_to_create, context=context)

        return invoice_errors, errors
    ...
    def compute_invoice_values(self, cr, uid, intrastat, invoice, parent_values, context=None):
        ...
        parent_values.setdefault('partner_country_id_to_write', False)
        if not parent_values['is_fiscal_only']:
            if not invoice.intrastat_transport:
                if not intrastat.company_id.default_intrastat_transport:
                    errors.append(
                        _("The mode of transport is not set on invoice '%s' "
                          "nor the default mode of transport on "
                          "the company '%s'.") %
                        (invoice.number, intrastat.company_id.name)
                    )
                else:
                ...
                if not intrastat.company_id.default_intrastat_department:
                    errors.append(
                        _("The intrastat department hasn't been set on "
                          "invoice '%s' and the default intrastat department "
                          "is missing on the company '%s'.") %
                        (invoice.number, intrastat.company_id.name)
                    )
                else:
            ...
            parent_values.update({
                'transaction_code_to_write': False,
                'partner_country_id_to_write': False,
            })
        return parent_values, errors
    ...
    def generate_product_lines_from_invoice(self, cr, uid, ids, context=None):
        ...
        if intrastat.type == 'import':
            ...
            invoice_type = 'in_invoice',  # must be a tuple
        if intrastat.type == 'export':
        ...
        for invoice in invoice_obj.browse(cr, uid, invoice_ids, context=context):
            ...
            if not partner_country:
                errors[invoice.id].append(_("Missing country on partner '%s'.") % invoice.partner_id.name)
            ...
            if not invoice.intrastat_country_id:
                if not partner_country.intrastat:
                    continue
                elif partner_country.id == intrastat.company_id.country_id.id:
                    continue
                else:
                    parent_values['partner_country_id_to_write'] = partner_country.id
            ...
            if not invoice.intrastat_type_id:
                if invoice.type == 'out_invoice':
                    if intrastat.company_id.default_intrastat_type_out_invoice:
                        ...
                        raise orm.except_orm(
                            ...
                            _("The intrastat type hasn't been set on "
                              "invoice '%s' and the 'default intrastat type "
                              "for customer invoice' is missing for "
                    ...
                    if intrastat.company_id.default_intrastat_type_out_refund:
                        ...
                        raise orm.except_orm(
                            ...
                            _("The intrastat type hasn't been set on refund "
                              ...
                              "customer refund' is missing for the company"
                              "'%s'.") % (invoice.number,
                                          intrastat.company_id.name)
                        )
                    ...
                    if intrastat.company_id.default_intrastat_type_in_invoice:
                        ...
                        raise orm.except_orm(
                            _('Error'),
                            _("The intrastat type hasn't been set on invoice "
                              "'%s' and the 'Default intrastat type for "
                              "supplier invoice' is missing for the company "
                              "'%s'.") % (invoice.number,
                                          intrastat.company_id.name)
                        )
                else:
            ...
            if invoice.intrastat_country_id and not partner_country.intrastat and invoice.partner_id.intrastat_fiscal_representative:
                # fiscal rep
            ...
            parent_values, vals_errors = self.compute_invoice_values(
                cr, uid, intrastat, invoice, parent_values, context=context)
            if vals_errors:
                errors[invoice.id] += vals_errors

            # message_post
            create_errors, create_line_errors = self.create_intrastat_product_lines(
                cr, uid, ids, intrastat, invoice,
                parent_values, context=context)

            if create_errors:
                errors[invoice.id] += create_errors
            if create_line_errors:
                line_errors[invoice.id] = create_line_errors

        if errors or line_errors:
            message_template = _(
                "<b>The generation of lines could not be completed "
                "due to the following issues:</b>"
                "<p>"
                "%s"
                "</p>"
            )
            ...
            for invoice_id in invoice_ids:
                line_message = []
                for line_id, line_errs in line_errors.get(invoice_id, {}).iteritems():
                    line = invoice_line_obj.browse(cr, uid, line_id,
                                                   context=context)
                    line_msg = lines_template % {
                        'name': line.name,
                        'errors': ''.join("<li>%s</li>" % err for err
                                          in line_errs)
                    }
                    line_message.append(line_msg)

                invoice = invoice_obj.browse(cr, uid, invoice_id,
                                             context=context)
                inv_errs = errors.get(invoice_id, [])
                if inv_errs or line_message:
                    invoice_message = invoice_template % {
                        'name': invoice.number,
                        'errors': ''.join("<li>%s</li>" % err for err
                                          in inv_errs),
                        'lines': ''.join(line_message),
                    }
                    invoice_messages.append(invoice_message)
            message = message_template % ''.join(invoice_messages)

            db = pooler.get_db(cr.dbname)
            local_cr = db.cursor()
            try:
                self.message_post(local_cr, uid, ids, body=message,
            ...
            raise orm.except_orm(
                _('Error'),
                _('The lines could not be generated, please '
                  'refresh the form and see the messages.')
            )
        return True
