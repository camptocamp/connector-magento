...
long as the build is green!) and push it on the registry as `camptocamp/debonix_odoo:R.r.x.y.z`
...
* Paul tests the image `camptocamp/debonix_odoo:12.0.1.4.0` and oops, it seems he
...
2. Create local patch branch
    ...
    * Create PR to patch remote project branch.
        ```git
        camptocamp/debonix_odoo/patch-R.r.x.y.next_z <- <user_name>/debonix_odoo/<your_patch_branch>
        ```
